...
import { SPEND_SEO, canonicalUrl, shareHead } from "@/lib/canonical";
import {
  SPEND_KINDS,
  SPEND_KIND_LABEL,
  daysUntil,
...
function SpendPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      <h1 className="text-title font-semibold">{lang === "en" ? "Monthly spend" : "月費開銳"}</h1>
      <p className="mt-3 text-sm leading-relaxed text-muted">
        {lang === "en"
          ? "Save your own broadband, mobile and OTT contracts on this device. Totals stay on your phone or computer — we do not upload them."
          : "自己登記寬頻、手機、OTT 合約，一次過睇月費同到期日。資料只存在呢部裝置，唔會上傳。"}
      </p>
      <p className="mt-2 text-sm leading-relaxed text-muted">
        {lang === "en"
          ? "Listed catalogue prices remain reference only."
          : "目錄月費仍只供參考，實際以查核報價為準。"}
      </p>
        ...
        <section className="mt-8 rounded-xl bg-surface px-4 py-5">
          ...
          {next && nextDays !== null ? (
            <p className="mt-3 text-sm text-muted">
              {lang === "en" ? "Next end date" : "最近到期"}：{next.provider || next.name || labels[next.kind]}{" "}
              {next.endDate}
              {nextDays < 0
                ? lang === "en"
                  ? ` (${Math.abs(nextDays)} days past)`
                  : `（已過 ${Math.abs(nextDays)} 日）`
                : lang === "en"
                  ? ` (${nextDays} days)`
                  : `（仲有 ${nextDays} 日）`}
            </p>
            ...
            <p className="mt-3 text-sm text-muted">
              {lang === "en" ? "Add an end date to see the next renewal." : "填合約到期日就會見到下一個續約。"}
            </p>
          )}
        </section>
      ) : null}
      ...
      <form className="mt-8 space-y-3" onSubmit={onSubmit}>
        ...
        <label className="block text-sm">
          ...
          <Input
            ...
            placeholder={lang === "en" ? "e.g. HKBN" : "例如 HKBN"}
          />
        </label>
        <label className="block text-sm">
      ...
      <section className="mt-10">
        ...
        {!ready ? (
          ...
          <p className="mt-3 text-sm text-muted">
            {lang === "en" ? "Nothing saved yet." : "未有記錄。加一項寬頻、手機或 OTT 就會出現呢下面。"}
          </p>
        ) : (
          grouped.map((group) => (
