...
function delayForNthChild(css: string, n: number) {
  const match = css.match(new RegExp(`\\.plan-list > \\*:nth-child\\(${n}\\)\\s*\\{([^}]+)\\}`));
  assert.ok(match, `missing .plan-list > *:nth-child(${n})`);
...
describe("plan list filter replay", () => {
  it("changes the remount key for estate, housing, speed, provider, and auto-filter fields", () => {
    ...
    assert.notEqual(
      planListReplayKey({ ...base, cat: "mobile" }),
      planListReplayKey({ ...base, cat: "broadband" }),
    );
  });
...
describe("plan list fade-up", () => {
  it("staggers a 240ms fade-up on list items without blocking clicks or foil", () => {
    ...
    assert.match(page, /key=\{planListReplayKey\(search\)\}/);
    assert.match(page, /className="plan-list /);
    ...
    assert.match(css, /\.plan-list > \*\s*\{[^}]*animation:\s*plan-card-in 240ms ease-out both/);
    assert.match(css, /\.plan-list > \*\s*\{[^}]*pointer-events:\s*auto/);
    assert.match(css, /\.plan-list > \*\s*\{[^}]*animation-delay:\s*450ms/);
    ...
    assert.equal(css.includes(".plan-list > *:nth-child(11)"), false);

    assert.match(
      css,
      /prefers-reduced-motion:\s*reduce[\s\S]*\.plan-list > \*[\s\S]*animation:\s*none !important;[\s\S]*transform:\s*none !important/,
    );
